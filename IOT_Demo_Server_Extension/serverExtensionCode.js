...
function add_secure_profile(params, context, done) {
  ...
  var security_group = admin.groupWithID("6fya1mdi88fehboc1kvw2nnv7");
...
function authority_modify(context, params, user, authority, done) {
  ...
  var queryCallbacks = {
    success: function (queryPerformed, resultSet, nextQuery) {
      ...
      obj.set(params.groupID, authority);
      obj.save({
        success: function (theObject) {
...
function insert_access_log(secure_profile, done) {
  ...
  $.ajax({
    ...
    headers: {
      'Authorization': 'Bearer akWPSVXNtsEX7cNDAjfEEhnD0FemtkLaTw5crEPdYw4',
...
function modify_access_log_acl(data, done, secure_profile) {
  var endpoint = Kii.getBaseURL() + "/apps/" + Kii.getAppID() +
    "/things/VENDOR_THING_ID:access_system/buckets/access_log/objects/" +
    data.objectID + "/acl/READ_EXISTING_OBJECT/GroupID:6fya1mdi88fehboc1kvw2nnv7";
  ...
  $.ajax({
    ...
    success: function () {
      ...
      $.ajax({
        type: "PUT",
        url: Kii.getBaseURL() + "/apps/" + Kii.getAppID() +
        "/things/VENDOR_THING_ID:access_system/buckets/access_log/objects/" +
        data.objectID + "/acl/READ_EXISTING_OBJECT/userID:" + secure_profile.UUID,
        headers: {
          'Authorization': 'Bearer akWPSVXNtsEX7cNDAjfEEhnD0FemtkLaTw5crEPdYw4',
          'X-Kii-AppID': Kii.getAppID(),
          'X-Kii-AppKey': Kii.getAppKey()
